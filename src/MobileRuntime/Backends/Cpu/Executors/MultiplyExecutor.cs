...
namespace MLFramework.MobileRuntime.Backends.Cpu.Executors
{
    ...
    /// <summary>
    /// Executor for element-wise multiplication.
    /// </summary>
    public sealed class MultiplyExecutor : IOperatorExecutor
    {
        private readonly CpuBackend _backend;

        public OperatorType OperatorType => OperatorType.Multiply;

        public MultiplyExecutor(CpuBackend backend)
        {
        ...
        public ITensor Execute(ITensor[] inputs, Dictionary<string, object> parameters)
        {
            if (inputs == null || inputs.Length != 2)
                throw new ArgumentException("Multiply requires exactly two input tensors.");
            ...
            unsafe
            {
                ...
                {
                    CpuVectorization.Multiply(outputPtr, input1Ptr, input2Ptr, data1.Length, _backend.IsVectorizationEnabled());
                }
            }
        ...
        {
            throw new NotSupportedException("MultiplyExecutor does not support operator fusion.");
        }
    }
}
