...
namespace MLFramework.LoRA
{
    ...
    public class LoRAConv2d : LoRAAdapterBase, IModule
    {
        ...
        public Tensor Forward(Tensor input)
        {
            ...
            int numPositions = patches.Shape[1];
            var patchesFlat = patches.Reshape(new[] { batchSize * numPositions, _loraA.Shape[1] });

            // Apply LoRA: A * patch [rank, in*k*k] x [batch*positions, in*k*k]
            var loraInput = MatMul(patchesFlat, _loraA.Transpose());
