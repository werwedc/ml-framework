...
/// <summary>
/// Gradient computation for division operation.
/// For z = x / y, dz/dx = 1/y and dz/dy = -x/y².
/// </summary>
public class DivGrad : IOperationGrad
{
    /// <inheritdoc/>
    public string OperationName => "Div";

    /// <inheritdoc/>
    public Tensor[] ComputeGrad(Tensor gradOutput, Tensor[] inputs, OperationContext context)
    {
        if (inputs.Length < 2)
            throw new ArgumentException("Div operation requires at least 2 inputs", nameof(inputs));

        var x = inputs[0];
        var y = inputs[1];

        // d(x / y)/dx = 1/y
        var gradX = ElementwiseMul(gradOutput, Reciprocal(y));

        // d(x / y)/dy = -x/y²
        var ySquared = ElementwiseMul(y, y);
        var xOverYSquared = ElementwiseDiv(x, ySquared);
        var negXOverYSquared = Negate(xOverYSquared);
        var gradY = ElementwiseMul(gradOutput, negXOverYSquared);
    ...
    /// <summary>
    /// Computes element-wise reciprocal (1/x) with numerical stability.
    /// </summary>
    private static Tensor Reciprocal(Tensor tensor)
    {
        ...
        for (int i = 0; i < newData.Length; i++)
        {
            ...
            if (Math.Abs(tensor.Data[i]) < epsilon)
            {
                newData[i] = tensor.Data[i] >= 0 ? float.PositiveInfinity : float.NegativeInfinity;
            }
            else
            {
                newData[i] = 1.0f / tensor.Data[i];
            }
        }
    ...
    /// </summary>
    private static Tensor ElementwiseDiv(Tensor a, Tensor b)
    {
        var broadcastedShape = BroadcastShape(a.Shape, b.Shape);
        var aBroadcasted = BroadcastTo(a, broadcastedShape);
        var bBroadcasted = BroadcastTo(b, broadcastedShape);
        ...
        {
            // Avoid division by zero
            if (Math.Abs(bBroadcasted.Data[i]) < epsilon)
            {
                resultData[i] = bBroadcasted.Data[i] >= 0 ? float.PositiveInfinity : float.NegativeInfinity;
            }
            else
            {
                resultData[i] = aBroadcasted.Data[i] / bBroadcasted.Data[i];
