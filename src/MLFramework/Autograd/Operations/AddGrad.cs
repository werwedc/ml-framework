...
public class AddGrad : IOperationGrad
{
    ...
    public Tensor[] ComputeGrad(Tensor gradOutput, Tensor[] inputs, OperationContext context)
    {
        ...
        // The gradient simply passes through unchanged
        var gradX = gradOutput.Clone();
        var gradY = gradOutput.Clone();

        // Handle broadcasting if necessary
        // If inputs had different shapes due to broadcasting, we need to reduce gradients back
    ...
    private static Tensor SumAlongAxis(Tensor tensor, int axis)
    {
        ...
        {
            // Convert flat index to multi-index in the reduced tensor
            var multiIdx = FlatToMultiIndex(i, newStrides);

            // Map to original tensor indices and sum
            float sum = 0;
