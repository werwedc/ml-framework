...
/// For z = sigmoid(x), dz/dx = sigmoid(x) * (1 - sigmoid(x)).
/// </summary>
public class SigmoidGrad : IOperationGrad
{
    /// <inheritdoc/>
    public string OperationName => "Sigmoid";

    /// <inheritdoc/>
    public Tensor[] ComputeGrad(Tensor gradOutput, Tensor[] inputs, OperationContext context)
    {
        if (inputs.Length < 1)
            throw new ArgumentException("Sigmoid operation requires at least 1 input", nameof(inputs));

        var x = inputs[0];

        // d(sigmoid(x))/dx = sigmoid(x) * (1 - sigmoid(x))
        // We need the output of sigmoid
        Tensor sigmoidX;
        if (context.HasSavedTensor("output"))
        {
            sigmoidX = context.GetSavedTensor<Tensor>("output");
        }
        else
        {
            // Fallback: compute sigmoid
            sigmoidX = ElementwiseSigmoid(x);
        }

        // Gradient = sigmoid(x) * (1 - sigmoid(x))
        var oneMinusSigmoid = ElementwiseSub(Tensor.Ones(sigmoidX.Shape), sigmoidX);
        var localGrad = ElementwiseMul(sigmoidX, oneMinusSigmoid);
    ...
    /// <summary>
    /// Computes element-wise sigmoid: 1 / (1 + exp(-x))
    /// </summary>
    private static Tensor ElementwiseSigmoid(Tensor tensor)
    {
        ...
        for (int i = 0; i < newData.Length; i++)
        {
            ...
            {
                newData[i] = 1.0f / (1.0f + (float)Math.Exp(-x));
            }
        }
