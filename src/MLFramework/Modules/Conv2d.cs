...
namespace MLFramework.Modules
{
    ...
    public class Conv2d : IModule
    {
        ...
        public Tensor Forward(Tensor input)
        {
            ...
            // We need: [batch, positions, inC*kH*kW] @ [inC*kH*kW, outC]
            var weightFlatTransposed = weightFlat.Transpose();
            var outputFlat = MatMul(patches, weightFlatTransposed);

            // Reshape output: [batch, out_h*out_w, outC] -> [batch, outC, out_h, out_w]
            var output = outputFlat.Reshape(new[] { batchSize, OutChannels, outHeight, outWidth });
                        ...
                        for (int c = 0; c < channels; c++)
                        {
                            for (int ky = 0; ky < kernelSize; ky++)
                            {
                                for (int kx = 0; kx < kernelSize; kx++)
                                {
                                    ...
                                    {
                                        output[new[] { b, oy, ox, c, ky, kx }] = input[new[] { b, c, iy, ix }];
                                    }
                                }
                            }
                        }
