...
{
    /// <summary>
    /// Static quantization implementation - weights and activations both quantized to Int8.
    /// </summary>
    public class StaticQuantization : IStaticQuantization
    {
        ...
        /// <summary>
        /// Creates a new StaticQuantization instance.
        /// </summary>
            ...
            List<string> skippedLayers)
        {
            ...
            var activationParams = _calibrationProcess.RunCalibration(model, dataLoader, config);
            ...
            foreach (var layer in quantizableLayers)
            {
                ...
                if (activationParams.ContainsKey(layerName))
                {
                    layerParams[$"{layerName}_activation"] = activationParams[layerName];
                }
            }
        ...
        private QuantizationParameters QuantizePerChannel(float[] weights, int[] shape, QuantizationConfig config)
        {
            ...
            return new QuantizationParameters(scale, (int)zeroPoint, avgMin, avgMax, config.WeightQuantization, config.QuantizationType);
        }
    }
}
