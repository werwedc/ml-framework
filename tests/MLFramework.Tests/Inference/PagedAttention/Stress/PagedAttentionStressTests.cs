...
/// <summary>
/// Stress tests for PagedAttention to verify system stability under heavy load
/// </summary>
...
public class PagedAttentionStressTests : IDisposable
{
    ...
    {
        // Run for a significant duration (e.g., 1 minute for faster CI runs)
    ...
    public void PoolExhaustion_GracefulFailure()
    {
        ...
        // Verify we allocated all blocks
        Assert.Equal(_blockManager.TotalBlocks, allocatedCount);
    ...
    {
        var data = new float[CalculateProduct(shape)];
        var random = new Random(42);
    ...
    private int CalculateProduct(int[] shape)
    {
        int product = 1;
...
/// <summary>
/// Memory leak detection tests for PagedAttention components
/// </summary>
[Collection("Sequential")] // Run sequentially to avoid interference
public class MemoryLeakTests : IDisposable
{
    ...
    public void TensorDisposal_NoMemoryLeak()
    {
        ...
        {
            var data = new float[32 * 128 * 128 * 128];
            var random = new Random(42);
            for (int j = 0; j < data.Length; j++)
            {
                data[j] = (float)random.NextDouble();
            }
            var tensor = new Tensor(data, new[] { 32, 128, 128, 128 }, _device);
            tensor.Dispose();
        }
    ...
    [Fact]
    public void ConcurrentBlockAllocation_NoMemoryLeak()
    {
        var initialMemory = GC.GetTotalMemory(true);

        var managers = new ConcurrentBag<KVCacheBlockManager>();
        var tasks = new List<Task>();

        // Create and dispose multiple block managers concurrently
        for (int t = 0; t < 10; t++)
        {
            int taskId = t;
            var task = Task.Run(() =>
            {
                for (int i = 0; i < 10; i++)
                {
                    ...
                    for (int j = 0; j < 5; j++)
                    {
                        int seqId = taskId * 1000 + i * 10 + j;
                        blockManager.AllocateBlocks(seqId, 10);
                        blockManager.FreeSequenceBlocks(seqId);
                    }
        ...
        var growth = finalMemory - initialMemory;
        Assert.True(growth < 200_000_000,
                   $"Memory growth: {growth / 1_000_000}MB");
    }
