# Spec: Block Table (Logical-to-Physical Mapping)

## Overview
Implement the BlockTable class that maintains the logical-to-physical mapping between token positions and memory blocks, enabling efficient non-contiguous KV cache access.
...
## Requirements
1. **Mapping Accuracy**: Correct mapping from (sequenceId, tokenIndex) to blockId
2. **Order Preservation**: Block lists must maintain order for a sequence
3. **Thread Safety**: Support concurrent operations from multiple sequences
4. **Efficient Lookup**: Fast lookups for token-to-block mapping
5. **Consistency**: BlockTable and BlockManager must stay in sync

## Testing Requirements
1. Unit tests for block append operations
2. Unit tests for token-to-block lookup accuracy
3. Unit tests for sequence length calculation
4. Unit tests for sequence removal
5. Unit tests for statistics calculation
6. Concurrent access tests (multiple sequences modifying their block lists)
7. Edge case tests (empty sequences, single token sequences)
...
## Success Criteria
- Accurate logical-to-physical mapping
- Fast lookup operations
- Consistent state with block manager
